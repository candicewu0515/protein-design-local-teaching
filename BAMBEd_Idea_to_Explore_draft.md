...
Two honest caveats are themselves part of the lesson. First, self-consistency pipelines often use AlphaFold2 as the folding oracle. We use ESMFold as the primary verifier because, as a single-sequence model needing no alignment, it is the fastest to run locally; we also ran a local AlphaFold2 (via ColabFold [4]) on the same GPU-free laptop for the four native sequences, so students can see the single-sequence vs. MSA-based contrast directly (Table 3). That comparison is itself a compute-visibility lesson: AlphaFold2's multiple-sequence-alignment step and per-length recompilation took roughly 1.8 hours for four small proteins on CPU, versus minutes for ESMFold. ESMFold is generally less accurate than AlphaFold2, especially on de novo sequences, so a self-consistent verdict here is weaker evidence than in a research screen [3]. Second, and more fundamentally, the screen validates self-consistency, **not** real-world function: an in silico pass is a hypothesis, not a working protein [1]. Teaching both caveats explicitly inoculates students against over-reading the result.
...
<sub>Optional fourth tool — **AlphaFold2** (via ColabFold): MSA-based validation, used here only as an enrichment comparison (Table 3). Runs GPU-free but is the heaviest validator on CPU—its alignment step and per-length recompilation took ~1.8 h for four small native sequences—so single-sequence ESMFold is the practical default in a GPU-free classroom.</sub>
...
**Table 3. Optional enrichment — the same four folds verified by single-sequence ESMFold vs. MSA-based AlphaFold2 (both run locally, GPU-free, on the native sequences).** Cα RMSD is to the experimental structure (chain A; metric definitions in the supplement).

| Protein (length) | ESMFold pLDDT | ESMFold Cα RMSD (Å) | AlphaFold2 pLDDT | AlphaFold2 pTM | AlphaFold2 Cα RMSD (Å) |
|---|---|---|---|---|---|
| Trp-cage (20 aa) | 91.1 | 0.51 | 94.9 | 0.13ᵃ | 0.69 |
| villin HP36 (36 aa) | 92.1 | 2.12 | 87.6 | 0.50 | 2.18 |
| protein G B1 (56 aa) | 88.3 | 0.49 | 96.8 | 0.81 | 0.29 |
| ubiquitin (76 aa) | 90.5 | 0.83 | 95.9 | 0.84 | 0.80 |

<sub>ᵃ pTM, like TM-score, is unreliable for very short chains—AlphaFold2's own pTM for the 20-aa Trp-cage is 0.13 despite a pLDDT of 94.9, a useful "no single metric" caution.</sub>

![Figure 1](teaching_demo/figures/fig_predictors_nature.png)

**Figure 1. Single-sequence ESMFold vs. MSA-based AlphaFold2 on the four native sequences, both run locally and GPU-free.** (**a**) Mean pLDDT (model confidence; dashed line marks the pLDDT = 90 "very high" threshold). (**b**) Cα-RMSD to the experimental structure (chain A; dashed line marks the 2 Å self-consistency threshold); villin is the hardest case for both predictors. (**c**) Per-residue pLDDT for each protein (blue, ESMFold; orange, AlphaFold2): single-sequence ESMFold dips where AlphaFold2's alignment holds confidence—at the villin termini, the protein G central strand, and the flexible C-terminal tail of ubiquitin. Numbers regenerate from `data_summary.csv` via `analyze_structures.py`; colour scheme is colour-blind safe (Okabe–Ito).

Three teaching points fall straight out of Table 3 and Figure 1. (i) **Independent confirmation of the hard case:** villin is the worst for both predictors (2.12 vs 2.18 Å) and AlphaFold2 *correctly* assigns it its lowest confidence (pLDDT 87.6), so the borderline verdict reflects the target, not one model. (ii) **MSA helps where homologs exist:** on protein G, alignment-based AlphaFold2 reaches 0.29 Å versus single-sequence ESMFold's 0.49 Å—a concrete demonstration of what the multiple-sequence alignment buys. (iii) **Heavier inputs cost compute:** that accuracy came at roughly 1.8 hours of CPU time for four small proteins (vs. minutes for ESMFold), making visible *why* a single-sequence model is the better default for a GPU-free classroom while AlphaFold2 remains an instructive optional comparison.
